...
tIntegerType ShiftLeft(tIntegerType a, int offset) {
  const int64_t wide_a = static_cast<int64_t>(a);
...
inline int16_t RoundingHalfSum(int16_t a, int16_t b) {
  ...
  return static_cast<int16_t>((sum + sign) / 2);
}
...
inline int16_t SaturatingAdd(int16_t a, int16_t b) {
  ...
  return static_cast<int16_t>(
      std::min(static_cast<int32_t>(32767),
               std::max(static_cast<int32_t>(-32768), sum)));
}
...
// for IntegerType==int32_t, the mapping is
...
template <>
inline int16_t SaturatingRoundingDoublingHighMul(int16_t a,
                                                      int16_t b) {
  ...
  int16_t ab_x2_high16 =
      static_cast<int16_t>((ab_32 + nudge) / (1 << 15));
...
inline typename FixedPointType::ScalarRawType RescaleConstantInitializer(
  ...
  return static_cast<ScalarRawType>(
      RoundingDivideByPOT<int32_t>(int32_value, 32 - ScalarTypeBits));
}
#ifdef GEMMLOWP_ENABLE_FIXEDPOINT_CONSTANTS_CHECKS
template <typename FixedPointType>
FixedPointType CheckedFixedPointConstant(int32_t raw_value,
                                         double double_value) {
